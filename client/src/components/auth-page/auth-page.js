...
export default class AuthPage extends Component {
  state = {
    isLogin: true,
    formValue: { name: '', surname: '', email: '', pass: '', confirm: '' },
  ...
  onFieldChange = (event, fieldName) => {
    ...
    switch (fieldName) {
      case 'name':
        formValue = { name: newValue, pass, confirm, email, surname }
        break
      case 'surname':
        formValue = { surname: newValue, pass, confirm, name }
        break
      case 'email':
  ...
  register = async (email, password, name, surname, confirm) => {
    if (password !== confirm) {
  ...
  login = async (email, password) => {
    try {
      const {
        ...
        status
      } = await this.apiService.auth('login', {
        email,
        password
      })
  ...
  onSubmit = async event => {
    ...
    const {
      isLogin,
      formValue: { email, pass, name, surname, confirm }
    } = this.state
    ...
    isLogin ? this.login(email, pass) : this.register(email, pass,name, surname, confirm)
  }
