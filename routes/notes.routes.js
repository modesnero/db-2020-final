...
router.post('/', async (req, res) => {
  try {
    ...
    // Save note in DB
    await newNote.save()
    res.status(201).json({ message: 'Заявление было успешно создана' })
  } catch (error) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.json({ message: 'Заявление было успешно удалено' })
  } catch (error) {
    res.status(500).json({ message: 'Ошибка сервера' })
  }
})
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.json({ message: 'Заявление было успешно обновлено' })
  } catch (error) {
    res.status(500).json({ message: 'Ошибка сервера' })
  }
})
